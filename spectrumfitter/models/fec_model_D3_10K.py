...
class Model:
    ...
    params = {
        'r': [0.1, 20000., 1000., 1, 'Ohm'],
        'c': [0.1, 1000, 200., 1e-15, 'fF'],
        'l': [0, 2000, 0., 1e-12, 'pH'],
        'gl': [0, 1000, 0., 1e-6, 'uS'],
        'ra': [0, 1000, 200., 1, 'Ohm'],
        'ca': [0, 1000, 0., 1e-15, 'fF'],
        'rasup': [0, 1000, 0., 1, 'Ohm']
    }
    ...
    def fit_default(self, f, y, checkboxes):
        ...
        params.add('ca', value=self.values['ca'], min=0., max=10e3, vary=checkboxes['ca'].isChecked())
    ...
    def fit_RCRa(self, base_f, base_y):
        ...
        # define masks
        masks = [base_f < 0.2e9]    # here we will only fit C
        masks += [base_f < 10e9]   # here we will only fit R
        ...
        for i, mask in enumerate(masks):
            ...
            # create fit parameters
            params = Parameters()
            params.add('c', value=self.values['c'], min=1e-15, max=1e-12, vary=True if i in [0, 2] else False)
            params.add('r', value=self.values['r'], min=1, max=100e3, vary=True if i in [1, 3, 4] else False)
            params.add('ra', value=self.values['ra'], min=1, max=10e3, vary=True if i in [4] else False)
    ...
    def fit_RC(self, base_f, base_y):       
        ...
        for i, mask in enumerate(masks):
            ...
            params = Parameters()
            params.add('c', value=self.values['c'], min=1e-15, max=1e-12, vary=True if i in [0,2] else False)
            params.add('r', value=self.values['r'], min=1, max=100e3, vary=True if i in [1,3] else False)
        
            # don't fit the following params
            # TODO: could generate these automatically
            params.add('ra', value=self.values['ra'], min=1, max=10e3, vary=False)
            params.add('l', value=self.values['l'], min=0., max=1e-6, vary=False)
            params.add('gl', value=self.values['gl'], min=0., max=1000e-6, vary=False)
            params.add('ca', value=self.values['ca'], min=0., max=1e-12, vary=False)
            params.add('rasup', value=self.values['rasup'], min=0., max=10e3, vary=False)
    ...
    def fit_RLCRa(self, base_f, base_y):
        ...
        for i, mask in enumerate(masks):
            ...
            params = Parameters()
            params.add('r', value=self.values['r'], min=100, max=100e3, vary=True if i in [0, 1] else False)
            params.add('l', value=self.values['l'], min=0., max=1e-6, vary=True if i in [2, 3] else False)
            params.add('c', value=self.values['c'], min=1e-15, max=1e-12, vary=True if i in [0, 1] else False)
            params.add('ra', value=self.values['ra'], min=1, max=10e3, vary=True if i in [1, 2, 3] else False)
